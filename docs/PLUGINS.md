...
| claude-md-management | claude-plugins-official | CLAUDE.mdの品質監査・改善・セッション学習の反映 |
...
| プラグイン | コマンド | 説明 |
|---|---|---|
| frontend-design | `/frontend-design` | 本番品質のUIコンポーネント・ページを生成 |
| claude-mem | `/claude-mem:mem-search` | 過去セッションの記憶をセマンティック検索 |
| claude-mem | `/claude-mem:make-plan` | 記憶を活用した実装計画を作成 |
| claude-mem | `/claude-mem:do` | 計画をサブエージェントで実行 |
| superpowers | `/superpowers:using-superpowers` | セッション開始時に必ず実行（スキル利用方法の確立） |
| superpowers | `/superpowers:brainstorming` | 実装前の要件深掘り（新機能・変更時は必須） |
| superpowers | `/superpowers:writing-plans` | 詳細な実装計画の策定 |
| superpowers | `/superpowers:executing-plans` | 計画をレビューチェックポイント付きで実行 |
| superpowers | `/superpowers:test-driven-development` | TDD（テストを先に書いてから実装） |
| superpowers | `/superpowers:dispatching-parallel-agents` | 独立タスクを並列サブエージェントに分散 |
| superpowers | `/superpowers:subagent-driven-development` | 現セッション内でサブエージェント駆動実装 |
| superpowers | `/superpowers:requesting-code-review` | コードレビューを依頼 |
| superpowers | `/superpowers:receiving-code-review` | コードレビューフィードバックを受け取る・精査 |
| superpowers | `/superpowers:verification-before-completion` | 「完了」宣言前の検証（必須） |
| superpowers | `/superpowers:finishing-a-development-branch` | ブランチ作業の完了（マージ・PR・クリーンアップ） |
| superpowers | `/superpowers:using-git-worktrees` | Gitワークツリーで作業を隔離 |
| superpowers | `/superpowers:systematic-debugging` | バグ・テスト失敗時の体系的デバッグ |
| superpowers | `/superpowers:writing-skills` | 新しいスキルの作成・編集・検証 |
| ralph-loop | `/ralph-loop:ralph-loop` | タスク完了まで自律ループを開始 |
| ralph-loop | `/ralph-loop:cancel-ralph` | 実行中のループをキャンセル |
| ralph-loop | `/ralph-loop:help` | ralph-loopのヘルプを表示 |
| claude-md-management | `/claude-md-management:revise-claude-md` | セッション学習内容をCLAUDE.mdに反映 |
| claude-md-management | `/claude-md-management:claude-md-improver` | CLAUDE.mdを監査・品質改善 |
...
### 概要
CLAUDE.mdファイルの品質監査・改善と、セッションで得た学習内容のCLAUDE.mdへの反映を行うプラグイン。リポジトリ内のすべてのCLAUDE.mdを評価し、品質レポートを出力して改善案を適用する。
...
```
セッション中に発見したプロジェクト固有のルール・パターン・設定をCLAUDE.mdに追記する。

#### CLAUDE.mdを監査・改善する
```
/claude-md-management:claude-md-improver
```
すべてのCLAUDE.mdをスキャンし、品質レポートを出力してから改善を適用する。

### 向いているケース
- セッションで学んだ慣習・設定をCLAUDE.mdに記録したいとき
- CLAUDE.mdの内容が古くなってきたと感じるとき
- 複数のCLAUDE.mdを一括で品質チェックしたいとき
...
### セッション終了時にCLAUDE.mdを更新するとき
1. `/claude-md-management:revise-claude-md` — 学習内容を反映
2. `/claude-md-management:claude-md-improver` — 品質監査・改善（定期的に実行）
